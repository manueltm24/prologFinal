...
sintoma(Sint):-pregunta('Se siente usted con',Sint).
pregunta(Obj, Sint):-known(Obj, Sint, true),!.
pregunta(Obj, Sint):-known(Obj, Sint, false),!, fail.
pregunta(Obj, Sint):-nl,write(Obj),write(' '),
...
diagnosticar:-nl,write('Diagnosticando enfermedad..........'),nl, enfermedad(sintoma,Enfermedad) ,!,nl,
			write('Usted posiblemente puede tener'), write(Enfermedad).
diagnosticar:- nl, write('Lo sentimos, no tenemos una enfermedad para tus sintomas!!').

start:-abolish(known/3),dynamic(known/3), retractall(known/3), diagnosticar,nl,nl, write('Intentar de nuevo ? (y/n)'),read(Resp),\+ Resp=y,
		nl,write('Bye ! Gracias por usar el sistema'),abolish(known,3) .
...
write('Responde a las preguntas que el sistema realize.'),nl,
nl.
...
symptom(Val):-ask('Se siente usted con',Val).
...
diagnose:-nl,write('Diagnosticando enfermedad..........'),nl,disease(symptom,Disease) ,!,nl,
			write('Usted posiblemente puede tener'), write(Disease).
diagnose:- nl, write('Lo sentimos, no tenemos una enfermedad para tus sintomas!!').

start:-menu,repeat, abolish(known/3),dynamic(known/3), retractall(known/3), diagnose,nl,nl, write('Intentar de nuevo ? (y/n)'),read(Resp),\+ Resp=y,
		nl,write('Bye ! Gracias por usar el sistema'),abolish(known,3) .
